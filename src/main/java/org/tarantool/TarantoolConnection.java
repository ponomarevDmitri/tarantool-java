...
public class TarantoolConnection extends TarantoolBase<List<?>> implements TarantoolSQLOps<Object,Long,List<Map<String,Object>>> {
    ...
    protected TarantoolBinaryPacket writeAndRead(Code code, Object... args) {
        try {
            ByteBuffer packet = BinaryProtoUtils
                    .createPacket(initialRequestSize, msgPackLite, code, syncId.incrementAndGet(), null, args);
            ...
            Long c = responsePackage.getCode();
            if (c != 0) {
                throw serverError(c, responsePackage.getBody().get(Key.ERROR.getId()));
            }
